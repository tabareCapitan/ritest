...
def permute_distribution(
    df: pd.DataFrame, formula: str, stat: str, permute_var: str, seeds: np.ndarray
) -> np.ndarray:
    """
...
def compute_p_value(dist: np.ndarray, obs_coef: float, alternative: str) -> float:
    """
...
def compute_p_value_ci(count: int, reps: int, method: str = "cp", alpha: float = 0.05) -> tuple:
    """
...
def invert_confidence_interval(
    ...
    grid = np.arange(
        obs_coef - ci_range * robust_se, obs_coef + ci_range * robust_se + ci_step, ci_step
    )
    profile = {}
...
def ritest(
    ...
    if ci == "CI":
        ci_coef, profile = invert_confidence_interval(
            df, formula, stat, permute_var, obs_coef, seeds, alternative, ci_range, ci_step
        )
