...
def ritest_set(overrides: dict):
    ...
    if invalid_keys:
        raise ValueError(
            f"Invalid config key(s): {invalid_keys}. Allowed keys: {list(DEFAULTS.keys())}"
        )
...
def ritest(
    ...
    *,
    ...
    def validate_inputs():
        ...
        if formula:
            ...
            if not pd.api.types.is_numeric_dtype(ymat[yvar_name]):
                raise ValueError(
                    f"The dependent variable '{yvar_name}' must be numeric, got {ymat[yvar_name].dtype}."
                )

            # RHS validation
            non_numeric_cols = [
                col for col in xmat.columns if not pd.api.types.is_numeric_dtype(xmat[col])
            ]
            if non_numeric_cols:
    ...
    if stat_fn:
        ...
        use_stat_fn = make_ols_stat_fn(cast(str, formula), cast(str, stat))
